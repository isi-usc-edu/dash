...
""")
    ...
    def setupAccount(self, service_type=None, service=None, requirements=None):
        ...
        if (self.password_list[desired_pass] + self.cognitiveBurden) < self.cogThreshold:
            ...
            #FIXME: stats is not defined (commited 584d5e0328eb4a5d7fad306aca919b658d7c8e3e https://github.com/cuts/webdash/commit/584d5e0328eb4a5d7fad306aca919b658d7c8e3e )
            stats = [] #FIXME tmp fix to make compilable
            password = max(iteritems(stats), key=operator.itemgetter(1))[0]
        else:
            password = desired_pass + "00"
            self.knownPasswords[password] = 1 # old version self.knownPasswords.append(password)
